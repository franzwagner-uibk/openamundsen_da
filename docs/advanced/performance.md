...
Use these outputs to identify sustained memory pressure, filesystem growth and
CPU under-utilization before changing the worker count. Temperature is recorded
only when a readable host sensor is exposed to the container.
