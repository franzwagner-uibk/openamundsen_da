...
The framework supports three ingest modes:

- `product_layer`: read value + uncertainty from the same NetCDF file.
- `companion_layer`: read uncertainty from `<source_stem>_uncertainty.tif`.
- `generated_layer`: same as companion-layer ingest, with companion rasters generated by the preprocessing tools.
  ...
  uncertainty:
    scf:
      ...
      ingest:
        ...
        # Required when mode=product_layer:
        scf_variable: fsc
      ...
      class_mapping:
        base_groups: [valid]
        max_uncertainty_groups: [cloud, water]
        nodata_groups: [nodata]
      penalties:
        - name: forest
          source: landcover
    ...
    wet_snow:
      ...
      nodata_value: 255.0
      class_mapping:
        base_classes: [110, 125]
        nodata_groups: [exclude]
```
