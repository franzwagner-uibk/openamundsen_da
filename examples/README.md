...
- The shipped project configurations use `humidity_perturbation_method: dew_point`, so `sigma_rh` is interpreted as a dew-point-temperature perturbation scale in K. Set `relative_humidity` explicitly only for legacy raw-RH perturbations.
