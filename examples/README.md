...
- The shipped `rofental` project uses the promoted `project_2022_2023` baseline: seed `113`, five station snow-depth events, one spring `wet_snow_line` event on `2023-03-24`, and two fSCA (`scf`) events on `2023-04-26` and `2023-05-26`. It keeps `wet_snow_fraction` as a diagnostic benchmark in the wet-snow summary outputs.
