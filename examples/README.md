...
- The shipped `rofental` project uses `humidity_perturbation_method: dew_point`, so `sigma_rh` is interpreted as a dew-point-temperature perturbation scale in K. The shipped `subdomains` project sets `relative_humidity` explicitly because its source meteo bundle contains legacy RH values above 100% that strict dew-point mode intentionally rejects.
