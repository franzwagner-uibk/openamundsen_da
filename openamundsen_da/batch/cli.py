"""CLI entrypoint for the batch subregion toolkit."""
...
def cli(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(prog="oa-da-batch", description="Batch subregion open-loop runner")
    sub = parser.add_subparsers(dest="command", required=True)

    p_prep = sub.add_parser("prepare", help="Prepare per-subregion setups from a regions file")
    ...
    p_prep.add_argument("--id-field", default="id", help="Field name containing subregion ID (default: id)")
    p_prep.add_argument("--clip-mode", choices=("window", "roi-symlink"), default="window", help="Grid handling mode")
    p_prep.add_argument("--station-buffer-km", type=float, default=10.0, help="Buffer (km) to include neighboring stations")
    ...
    p_run = sub.add_parser("run", help="Run open-loop simulations for prepared subregions")
    ...
    p_run.add_argument("--subregions", nargs="+", help="Optional list of subregion IDs to run")
    p_run.add_argument("--max-workers", type=int, help="Max parallel workers")
    p_run.add_argument("--retries", type=int, default=1, help="Retries per subregion on failure (default: 1)")
    ...
    p_merge = sub.add_parser("merge", help="Merge outputs across subregions")
    ...
    p_merge.add_argument("--subregions", nargs="+", help="Optional list of subregion IDs to merge")
    p_merge.add_argument("--mode", choices=("hard_clip", "blend"), default="hard_clip", help="Merge mode for grids")
    ...
    p_pipe.add_argument("--id-field", default="id", help="Field name containing subregion ID (default: id)")
    p_pipe.add_argument("--clip-mode", choices=("window", "roi-symlink"), default="window", help="Grid handling mode")
    p_pipe.add_argument("--station-buffer-km", type=float, default=10.0, help="Buffer (km) to include neighboring stations")
    ...
    p_pipe.add_argument("--retries", type=int, default=1, help="Retries per subregion on failure (default: 1)")
    p_pipe.add_argument("--mode", choices=("hard_clip", "blend"), default="hard_clip", help="Merge mode for grids")
    ...
    p_pipe.add_argument("--overwrite", action="store_true", help="Overwrite setups and rerun subregions")
