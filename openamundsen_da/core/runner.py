...
def run_member(
    ...
    *,
    ...
    try:
        ...
        is_first_step = str(step_name).startswith("step_00")
        if is_first_step:
            do_restart = False
            do_dump = True
        ...
        model.initialize()
        # Warm start (strict for steps >= 01): copy state via member-root pointer
        if do_restart:
            state_file = _resolve_state_file(results_dir)
            if state_file is None:
                raise RuntimeError(f"Warm start required but state pointer missing/invalid under {results_dir.parent}")
