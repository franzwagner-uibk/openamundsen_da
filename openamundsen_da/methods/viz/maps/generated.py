    ...
    return GeneratedRow(
        ...
        panels=(
            ...
            MapPanelSpec(
                ...
                title="Posterior mean snow depth",
                show_hillshade=True,
...
def _fraction_row(*, row: int, kind: str, label: str) -> GeneratedRow:
    ...
    return GeneratedRow(
        ...
        panels=(
            ...
            MapPanelSpec(kind=kind, row=row, col=3, title="Observed WSF"),
        ),
    )
...
def _generated_rows_for_event(project_dir: Path, event: AssimilationEvent) -> tuple[GeneratedRow, ...]:
    ...
    if event.variable == "station_hs":
        ...
        rows.append(_fraction_row(row=row_index, kind="wet_snow", label=_stream_row_label("wet_snow_line")))
        row_index += 1
